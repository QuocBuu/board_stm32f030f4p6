...
void ar_game_archery_display() {
	if (archery.visible == true && settingsetup.num_arrow != 0) {
	...
	else if (archery.visible == true && settingsetup.num_arrow == 0) {
		view_render.drawBitmap(	archery.x, \
								archery.y - 10, \
								bitmap_archery_II, \
								SIZE_BITMAP_ARCHERY_X, \
...
#define AR_GAME_TIME_TICK_SETUP() \
do { \
...
#define AR_GAME_SAVE_AND_RESET_SCORE() \
do { \
...
#define AR_GAME_TIME_TICK_REMOVE() \
do { \
